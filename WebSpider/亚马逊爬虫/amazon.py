# -*- coding: utf-8 -*-
"""
Created on Tue Oct 15 17:28:00 2019
...
def gen_xls(item_infos):
    ...
    worksheet.write_row(0,0, ['关键词','排名','宝贝图片','价格','宝贝类目','宝贝描述','宝贝链接','图片链接','宝贝评论','宝贝好评'])
    ...
    for i in range(len(item_infos)):
        ...
        try:
            ...
            row =   [item_info['keyword'],item_info['rank'],'',item_info['price'],item_info['cat'],item_info['descriptions'],item_info['item_url'],item_info['item_pic_base64'],item_info['comments'],item_info['histograms']]
            ...
            if 'item_pic_base64' in item_info:
                ...
                try:
                    ...
                    with open('test.jpg', 'wb') as f:
                        f.write(data)
                    img_resize('test.jpg', 'img/tmp%s.jpg'%i)
                    worksheet.insert_image( col,2, 'img/tmp%s.jpg'%i) # 名字必须不同
...
if __name__ == '__main__':
    ...
    item_infos = []
    cop = ChromeOperate()
    cop.open('https://www.amazon.com/')
    with open('keywords','r') as keyword_file:
        for line in keyword_file:
            line = line.strip()
            if not line:
                continue
            #urls ='https://www.amazon.com/s?k=andalou&page=2&qid=1571571333&ref=sr_pg_1'
